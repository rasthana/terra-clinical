...
module.exports = {
  ...
  'Displays a provider embedded in the consumer that auto resizes based on content.': (browser) => {
    const consumerSrc = '#/tests/embedded-content-consumer-tests/resize-consumer';
    const providerSrc = '#/tests/embedded-content-consumer-tests/resize-provider';

    browser.url(`http://localhost:${browser.globals.webpackDevServerPort}/${consumerSrc}`)
    // Wait for the consumer to load the provider.
    .waitForElementPresent(`iframe[src*="${providerSrc}"]`, 1000);
    // Validate the default component height.
    browser.expect.element(`iframe[src*="${providerSrc}"]`).to.have.css('height').which.equals('92px');
    // Pause the test briefly till the component has resized.
    browser.pause(3000);
    // // Validate the component height is changed after resizing.
    // .expect.element('iframe').to.have.css('height').which.does.not.equals('92px');
  },
};
